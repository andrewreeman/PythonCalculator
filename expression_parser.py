...
class ExpressionParser:
	...
	def parse(self, stream):		
		...
		while stream.hasChars() and not stream.peek() == ')':				
			...
			if not self._numberParser.canConsume(stream) and not self._operatorParser.canConsume(stream) and not stream.peek() == '(':
				stream.next()
				continue
						
			numberToken = self._numberParser.parse(stream)			
			if numberToken:				
				self._stack.pushNumber(numberToken)					

			operatorToken = self._operatorParser.parse(stream)
			if not operatorToken and numberToken:
				operatorToken = numberToken.convert_to_subtract_operator()
				if operatorToken:
					self._stack.popNumber()
			...
			if stream.peek() == '(':
				...
				bracketTree = ExpressionParser(newStack, self._logic, self._numberParser, self._operatorParser).parse(stream)
